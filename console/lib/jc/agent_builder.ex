defmodule Jc.AgentBuilder do
  ...
  @tool_lib %{
    ...
    "write_file" => "tool write_file(path: String, content: String) do |p, c| jet_fs::write(p, c) end",
    "edit_file" => "tool edit_file(path: String, old: String, new: String) do |p, o, n| jet_fs::edit(p, o, n) end",
  ...
  def codegen(list) do
    ...
    catalog = list |> Enum.map(fn a -> "{:#{akey(a)}, #{jstr(a["label"] || a["key"])}}" end) |> Enum.join(",\n     ")
    clauses = list |> Enum.map(fn a -> "      case :#{akey(a)}\n        custom_agents::#{mod(a)}.spawn()" end) |> Enum.join("\n")
  ...
  defp runner_decl(a) do
    ...
    case a["runner"] do
      "Fleet" ->
        opts = compact([members_opt("members", a), "reduce: #{jstr(a["accept"] || "Synthesize the members' outputs into one clear answer.")}", bk])
        "Fleet(#{opts})"
      ...
      "Debate" ->
        opts = compact([members_opt("agents", a), "rounds: #{int(a["max_rounds"]) || 2}", "judge: {role: #{jstr("Weigh the sides and give a decisive verdict.")}}", bk])
        "Debate(#{opts})"
  ...
  defp routed_decl(a) do
    ...
    acc = if x = nonempty(a["accept"]), do: "accept: #{jstr(x)}", else: "accept: #{jstr("The reply directly and correctly answers the request.")}"
  ...
  defp members_opt(key, a) do
    ms =
      (a["members"] || [])
      |> Enum.map(fn m -> "{name: #{jstr(m["name"] || "Member")}, role: #{jstr(m["role"] || "Do your part.")}}" end)
      |> Enum.join(",\n                 ")
  ...
  defp two_members(a) do
    ...
    c = (Enum.at(ms, 1) || %{})["role"] || "Critique the draft against the task; reply APPROVED when it fully satisfies it."
    [w, c]
  end
  ...
  defp akey(a), do: a["key"] |> to_string() |> String.replace(~r/[^a-zA-Z0-9_]/, "_") |> String.downcase()
