...
エディタから動かす方法は[こちら](#エディタの中のエージェントacp-経由)、[Jet Console](#あるいは-web-ui-でjet-console) はこちら。
...
この議論の正直な版です。都合の良い版は、Elixir を知っている人との接触に耐えないので。
...
どれも賢いことはしていません。ただ**選択の余地がない**だけです — そして*選択の余地がない*ことこそ、ライブラリには売れず言語だけが売れるものです。

記法はこの判断を正当化するのではなく、そこから帰結します。エージェントが第一級の形式になれば、値とプロセスとエージェントを貫く `x.method()` は、そのオブジェクトモデルの見た目そのものになります。[`expose`](#エージェント) は `actor` と `agent` で文字どおり同じキーワードで、違いは戻り値の型だけです。
...
- **`expose m(args)`** — `actor` と共有する唯一の宣言形式。戻り値の型が配送方法を決めるので、覚えるべき second keyword はありません:
  - `->` なし — 自由テキスト。
...
「JSON だけで答えよ」と言われたモデルは、それでも markdown フェンスで包み、先に考えを述べ、`'` でクォートし、カンマを落とし、数値を文字列で返します。Jet はやり直しを求めません — リトライは往復のコストを払ったうえで、大抵は同じ出力を再生産するので。**宣言されたスキーマに照らして応答を修復します。**
...
これは全 runner・全シェイプで動きます — [`src/jet_sap.jet`](src/jet_sap.jet)、[BAML](https://boundaryml.com/blog/schema-aligned-parsing) のリファレンス実装の設計から移植したものです。そのテストスイートは「モデルが実際にやること」の一覧です: `./jet -r jet_sap::run_tests src/jet_sap.jet`。
...
**`?`** は飾りではなく、値段を変えます。*必須*フィールドの欠落は上のスコアで 100、*任意*フィールドの欠落は 1。この差がこの記号の意味そのものです — 「モデルが正当に省略した」が「答えが間違っている」と同じ事象であることをやめます。`null` は欠落として扱われます。
